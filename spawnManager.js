module.exports = {
	...
	createSpawnCensus: function(room) {
		...
		for (let i in searchRooms) {
			...
			let itr = Game.rooms[searchRooms[i]]
	...
	createCreep: function(spawn, name, data, partNumber) {
		...
		let error = spawn.createCreep(parts, this.getName(name), data);
		if (typeof(error) === 'string') {
			let logStr = `${spawn.room.name} spawning ${error} in ${parts.length*3} ticks with ${spawn.name}. ` + JSON.stringify(data);
			log(logStr);
